...
# ========================================================================================================
# POST method - Resume the specified scheduled job - pentaho/api/scheduler/resumeJob
# ========================================================================================================
Scenario: A valid user calls web service to resume a specified scheduled job in XML format
	Given a scheduled job already exists
	And the job state is "Paused"
	When a HTTP POST request sent to "pentaho/api/scheduler/resumeJob" 
	...
	And the job status is changed from "Paused" to "Normal"
...
Scenario: A valid user calls web service to resume a specified scheduled job in JSON format
	Given a scheduled job already exists
	And the job state is "Paused"
	When a HTTP POST request sent to "pentaho/api/scheduler/resumeJob" 
...
Scenario: Admin user calls web service to resume a specified scheduled job which is already in "Normal" state
	Given a scheduled job already exists
...
Scenario: Admin user calls web service to resume a specified scheduled job in XML format but the job id is invalid
	Given the job id is invalid "admin	AdminScheduleTest16	34353212dfewr"
	When a HTTP POST request sent to "pentaho/api/scheduler/resumeJob" 
	And the Content-Type header is set to "application/xml"
...
Scenario: Pat user calls web service to resume a specified scheduled job created by Suzy in XML format
...
# Note: API call returned 200 OK response code which is not expected. But expecting 403 as Pat user is not authorized to view/resume the paused scheduled job created by Suzy.

Scenario: A non-existent user calls web service to resume a specified scheduled job
