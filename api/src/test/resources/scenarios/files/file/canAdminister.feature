...
# GET Method
Scenario Outline: Created user calls the web service to check if they can administer platform. #TC1
	Given created user with assigned role that have Operation Permissions <rolePermissions>
	When is called the web service canAdminister
	Then the status code is 200
	And the response body equals <responseBody>
Examples:
	...
	| create                                                                     | false        | Single Operation Permission create             | #TC1.5
	...
	| scheduler,read,publish,create,execute                                      | false        | Role Power User                                | #TC1.9
	| scheduler,publish                                                          | false        | Role Report User                               | #TC1.10
	| administerSecurity,read,create                                             | true         | Permissions combinantion                       | #TC1.11
	| "empty/null"                                                               | false        | No Operation Permission assigned to Role       | #TC1.12

Scenario: Created user with no role assigned calls the web service to check if they can administer platform. #TC2
	Given created user with no assigned role
	When is called the web service canAdminister
	Then the status code is 200
	And the response body equals "false"

Scenario Outline: Invalid user calls the web service canAdminister to check if they can administer platform. #TC3
