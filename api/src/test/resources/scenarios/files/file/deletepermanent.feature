...
# Scenarios for files
Scenario Outline: <User> calls the web service to permanently delete the file <Path>
	Given the file <Path> exists
	When <User> calls the web service
	Then the status code is 200
	And the file no longer exists at <Path>
	And the file does not exist in the trash folder

Scenario: The admin user calls the web service to delete multiple files in a single request.
	Given the file "/home/admin/DeleteMePerm.txt" exists
	And the file "/public/DeleteMePerm.txt" exists
	When the user calls the web service to delete the files using the file IDs in the params parameter
	Then the status code is 200
	And the "DeleteMePerm.txt" file no longer exists in "/home/admin"
	And the "DeleteMePerm.txt" file no longer exists in "/public"
	And the "DeleteMePerm.txt" files are not in the trash folder
	
Scenario: The suzy user calls the web service to delete a file.
	Given the file "/home/suzy/SuzyFilePerm.txt" exists
	And the suzy user has read permissions
	When the user calls the web service
	Then the response code is 200
	And the file "/home/suzy/SuzyFilePerm.txt" no longer exists
	And the file "SuzyFilePerm.txt" is not in the trash folder
...
Scenario Outline: <User> calls the web service to permanently delete the folder <Path>
	Given the folder <Path> exists
	When <User> calls the web service
	Then the status code is 200
	And the folder no longer exists at <Path>
	And the folder does not exist in the trash folder
	
Scenario: The admin user calls the web service to delete multiple folders in a single request.
	Given the folder "/home/admin/DeleteMePerm" exists
	And the folder "/public/DeleteMePerm" exists
	When the user calls the web service to delete the folders using the IDs in the params parameter
	Then the status code is 200
	And the "DeleteMePerm" folder no longer exists in "/home/admin"
	And the "DeleteMePerm" folder no longer exists in "/public"
	And the "DeleteMePerm" folders are not in the trash folder
	
Scenario: The suzy user calls the web service to delete a folder.
	Given the folder "/home/suzy/SuzyFilePerm" exists
	And the suzy user has read permissions
	When the user calls the web service
	Then the response code is 200
	And the folder "/home/suzy/SuzyFilePerm" no longer exists
	And the folder "SuzyFilePerm" is not in the trash folder
...
# This folder should not actually exist due to invalid characters in the name.
Scenario: The admin user calls the web service to delete a folder with invalid special characters in its name.
	When the user calls the web serivce for the folder "/\.prpti"
	Then the status code is 400
	And the response body is empty