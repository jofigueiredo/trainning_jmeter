...
# Note: This endpoint is similar to startExec. However, this endpoint is the equivalent to the combination of prepareExec and startExec.
...
Scenario: The user calls the web service to execute a transformation using the name parameter.
	...
	And the response body indicates that the request is successful in HTML format
...
Scenario Outline: The user calls the web service to execute a transformation using the name and xml parameters.
	...
	Then the status code is 200
	And the transformation is executed
	And the response body indicates that the request is successful in <responseFormat> format
Examples:
...
Scenario: The user calls the web service to execute a transformation that has been previously executed.
	...
	Then the status code is 200
	And the specified transformation is executed
	
Scenario: The user calls the web service to execute a transformation that has been prepared using the prepareExec endpoint.
	Given the transformation is present on Carte server
...
Scenario: The user calls the web service to execute a transformation by <Parameters>, but the transformation does not exist.
