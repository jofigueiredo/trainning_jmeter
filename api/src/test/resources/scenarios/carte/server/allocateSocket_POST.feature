...
Feature: /kettle/allocateSocket
  ...
  This API endpoint allows any client to ask for a port number to use for a Carte server using a HTTP POST request

Background:
  Given a default Carte slave server is actively running

@POST
Scenario Outline: An unauthorized user attempts to allocate a socket on a Carte server with a missing required parameter using POST
  ...
  And the "rangeStart" parameter is set to <rangeStart>
  And the "host" parameter is set to <ipAddress>
  And the "id" parameter is set to <carteContainerId>
  And the "trans" parameter is set to <transformationName>
  And the "sourceSlave" parameter is set to <sourceSlaveServerName>
  And the "sourceStep" parameter is set to <sourceStepPortNumber>
  And the "sourceCopy" parameter is set to <sourceStepCopyCount>
  And the "targetSlave" parameter is set to <targetSlaveServerName>
  And the "targetStep" parameter is set to <targetStepPortNumber>
  And the "targetCopy" parameter is set to <targetStepCopyCount>
  When a HTTP POST request is sent to "/kettle/allocateSocket" with valid parameters
...
@POST
Scenario: An unauthorized user attempts to allocate a socket on a Carte server with accepted parameter combinations using POST
  ...
  And the "xml" parameter is set to "n"
  And the "rangeStart" parameter is set to 12300
  And the "host" parameter is set to "localhost"
  And the "id" parameter is set to "id_valid"
  And the "trans" parameter is set to "trans_valid"
  And the "sourceSlave" parameter is set to "sourceSlave_valid"
  And the "sourceStep" parameter is set to 10
  And the "sourceCopy" parameter is set to 1
  And the "targetSlave" parameter is set to "targetSlave_valid"
  And the "targetStep" parameter is set to 10
  And the "targetCopy" parameter is set to 1
  When a HTTP POST request is sent to "/kettle/allocateSocket" with valid parameters
  Then the response status code will be 401
  And the response will contain "Error 401 Unauthorized"
...
@POST
Scenario Outline: An authorized user attempts to allocate a socket on a Carte server with missing required parameters using POST
...
@POST
Scenario Outline: An authorized user attempts to allocate a socket on a Carte server with an invalid port number using POST
  Given a valid encode for the Basic HTTP Authorization Request Header using the username "cluster" and the password "cluster"
  And the "xml" parameter is set to "N"
  And the "rangeStart" parameter is set to <invalidPort>
  And the "host" parameter is set to "localhost"
  And the "id" parameter is set to "id_valid"
  And the "trans" parameter is set to "trans_valid"
  And the "sourceSlave" parameter is set to "sourceSlave_valid"
...
@POST
Scenario Outline: An authorized user attempts to allocate a socket on a Carte server with accepted parameter combinations using POST
  Given a valid encode for the Basic HTTP Authorization Request Header using <username> and <password>
  And the "xml" parameter is set to <xml>
  And the "rangeStart" parameter is set to <rangeStart>
  And the "host" parameter is set to <ipAddress>
