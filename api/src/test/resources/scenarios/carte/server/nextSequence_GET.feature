...
Scenario: An unauthorized user attempts to get a sequence from the Carte server without specifying any parameters
  ...
  When a HTTP GET Request is sent to "/kettle/nextSequence"
  Then the response status code will be 401
  And the response will contain "Error 401 Unauthorized"

@GET
Scenario: An unauthorized user attempts to get an existing sequence from the Carte server using only the name URL parameter
  Given an invalid encode for the Basic HTTP Authorization Request Header using <username> and <password>
  When a HTTP GET Request is sent to "/kettle/nextSequence"
  And the "name" URL parameter is set to "testSeq"
  Then the response status code will be 401
  And the response will contain "Error 401 Unauthorized"

@GET
Scenario: An unauthorized user attempts to get an existing sequence from the Carte server using the name and increment URL parameters
  Given an invalid encode for the Basic HTTP Authorization Request Header using <username> and <password>
  When a HTTP GET Request is sent to "/kettle/nextSequence"
  And the "name" URL parameter is set to "testSeq"
  And the "increment" URL parameter is set to "1000"
  Then the response status code will be 401
  And the response will contain "Error 401 Unauthorized"
...
@GET
Scenario: An authorized user attempts to get a nonexistent sequence from the Carte server using only the name URL parameter
  Given a valid encode for the Basic HTTP Authorization Request Header using <username> and <password>
  When a HTTP GET Request is sent to "/kettle/nextSequence"
  And the "name" URL parameter is set to "seqDoesNotExist"
  Then the response status code will be 404
  And the response will contain "Slave sequence &#39;seqDoesNotExist&#39; could not be found."

@GET
Scenario Outline: An authorized user attempts to get an existing sequence from the Carte server using only the name URL parameter
  Given a valid encode for the Basic HTTP Authorization Request Header using <username> and <password>
  When a HTTP GET Request is sent to "/kettle/nextSequence"
  And the "name" URL parameter is set to "testSeq"
...
@GET
Scenario Outline: An authorized user attempts to get an existing sequence from the Carte server using the name and increment URL parameters
  Given a valid encode for the Basic HTTP Authorization Request Header using <username> and <password>
  When a HTTP GET Request is sent to "/kettle/nextSequence"
  And the "name" URL parameter is set to "testSeq"
  And the "increment" URL parameter is set to <incrementBy>
  Then the response status code will be 200
  And the response "<value>" field will contain <responseValue>
  And the response "<increment>" field will contain <responseIncrement>
