...
Feature: /kettle/listSocket
  ...
  This API endpoint allows any client to list the sockets used on a Carte server using a HTTP POST request.
...
Scenario: An unauthorized user attempts to list all the sockets of a Carte server without specifying any parameters
  ...
  When a HTTP POST Request is sent to "/kettle/listSocket"
  Then the response status code will be 401
  And the response will contain "Error 401 Unauthorized"

@POST
Scenario Outline: An unauthorized user attempts to list all the sockets of a Carte server with an abnormal host parameter
  Given an invalid encode for the Basic HTTP Authorization Request Header using the username "admin" and the password "password"
  When a HTTP POST Request is sent to "/kettle/listSocket"
  And the "host" parameter is set to <hostAddress>
  Then the response status code will be 401
  And the response will contain "Error 401 Unauthorized"
...
@POST
Scenario: An unauthorized user attempts to list all the sockets of a Carte server with a valid host parameter
  Given an invalid encode for the Basic HTTP Authorization Request Header using the username "admin" and the password "password"
  When a HTTP POST Request is sent to "/kettle/listSocket"
...
@POST
Scenario Outline: An authorized user attempts to list all the sockets of a Carte server with an abnormal host parameter
...
@POST
Scenario Outline: An authorized user attempts to list all the sockets of a Carte server with a standard host parameter
...
@POST
Scenario Outline: An unauthorized user attempts to list only the open sockets of a Carte server with a standard host parameter
  Given an invalid encode for the Basic HTTP Authorization Request Header using <username> and <password>
  When a HTTP POST Request is sent to "/kettle/listSocket"
  And the "host" parameter is set to <hostAddress>
  And the "onlyOpen" parameter is set to "Y"
  Then the response status code will be 401
  And the response will contain "Error 401 Unauthorized"
...
@POST
Scenario Outline: An authorized user attempts to list only the open sockets of a Carte server with a standard host parameter
  Given a valid encode for the Basic HTTP Authorization Request Header using <username> and <password>
  When a HTTP POST Request is sent to "/kettle/listSocket"
  And the "host" parameter is set to <hostAddress>
  And the "onlyOpen" parameter is set to "Y"
...
Scenario Outline: An authorized user attempts to list all the sockets of a Carte server with a single socket allocated to it via the API
  ...
  When a HTTP POST Request is sent to "/kettle/listSocket"
  And the "host" parameter is set to <hostAddress>
  And the "onlyOpen" parameter is set to <isOnlyOpen>
  Then the response status code will be 200
